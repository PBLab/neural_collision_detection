...
def create_column_graph(collision_arr, resolution, fname):
	...
	for i in xrange(len(collision_arr)):
		for j in xrange(len(collision_arr[0])):
			for k in xrange(len(collision_arr[0][0])):
				cur_collisions = collision_arr[i][j][k]
				if max_collisions < cur_collisions:
					max_collisions = cur_collisions

	graph = []
	graph.append(range(0, max_collisions, resolution))
